...
// static assert
#define NV_COMPILE_TIME_ASSERT(exp) static_assert(exp,#exp)
#define NV_COMPILE_TIME_ASSERT_REASON(exp,reason) static_assert(exp,reason)
...
template <class ... Args>
NV_CUDA_CALLABLE NV_INLINE void NV_UNUSED(Args const& ...)
{
}
#else
#define NV_UNUSED(...) (void)(__VA_ARGS__);
#endif
...
#if NV_WINDOWS_FAMILY
#define NV_COMPILE_TIME_MESSAGE(x) __pragma(message("**:"x));
#define NV_TODO(author,reason) __pragma(message(__FILE__ "(" NV_STRINGIZE(__LINE__) ") : **TODO**: " author " : " reason))
// Use this macro if you want to make a TODO note but *don't* want it to generate compile time log spew; useful for TODO items in header files
#define NV_TODO_SILENT(author,reason)
#else
#define NV_COMPILE_TIME_MESSAGE(x)
#define NV_TODO(author,reason)
#define NV_TODO_SILENT(x,y)
#endif
